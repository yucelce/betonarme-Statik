...
interface Props {
  state: AppState;
  onElementSelect?: (id: string | null) => void; // Yeni prop
  selectedElementId?: string | null; // Yeni prop
}

const Visualizer: React.FC<Props> = ({ state, onElementSelect, selectedElementId }) => {
        ...
        <svg width="100%" height="100%" viewBox={`0 0 ${canvasSize} ${canvasSize}`} className="bg-slate-50/50">
          ...
          <g {...interactiveGroupProps}>
            ...
            {model.beams.map(beam => {
              ...
              const isSelected = selectedElementId === beam.id;

              return (
                <g key={beam.id} onClick={() => handleMouseClick(beam.id)} className="cursor-pointer group">
                  {/* Tıklama alanını genişletmek için görünmez kalın çizgi */}
                  <line
                    x1={startX + toPx(n1.x)} y1={startY + toPx(n1.y)}
                    x2={startX + toPx(n2.x)} y2={startY + toPx(n2.y)}
                    stroke="transparent"
                    strokeWidth={beamW + 10}
                  />
                  {/* Görünür Kiriş */}
                  <line
                    x1={startX + toPx(n1.x)} y1={startY + toPx(n1.y)}
                    x2={startX + toPx(n2.x)} y2={startY + toPx(n2.y)}
                    stroke={isSelected ? "#2563eb" : "#e9d5ff"} // Seçiliyse Mavi
                    strokeWidth={beamW}
              ...
              return (
                <g key={`floor-${floorIndex}`}>
                  ...
                  {model.beams.map(beam => {
                    ...
                    // BURASI ÖNEMLİ: 3D dönüşüm (toIso) kullanıyoruz
                    ...
                    return (
                      <g key={`b3d-${beam.id}-${floorIndex}`} onClick={() => handleMouseClick(beam.id)} className="cursor-pointer group">
                        {/* Geniş tıklama alanı (Görünmez çizgi) */}
                        <line
                          x1={start.x} y1={start.y} x2={end.x} y2={end.y}
                          stroke="transparent" strokeWidth={(beamW * 0.8) + 10}
                        />
                        {/* Görünür Kiriş */}
                        <line
                          x1={start.x} y1={start.y} x2={end.x} y2={end.y}
                          stroke={isSelected ? "#2563eb" : "#f97316"} // Seçiliyse Mavi, değilse Turuncu
                          strokeWidth={beamW * 0.8}
                          className="transition-colors duration-200 group-hover:stroke-blue-400"
                        />
                      </g>
                    );
