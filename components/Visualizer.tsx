...
const Visualizer: React.FC<Props> = ({ dimensions, sections }) => {
  ...
  // Yapının ekrandaki toplam yüksekliğini hesapla
  const totalDrawingHeight = totalHeight * isoScale;
  // Canvas merkezinden, yapı yüksekliğinin yarısı kadar aşağı in (Taban noktası)
  // 0.9 katsayısı ile görseli hafifçe yukarı itiyoruz (göz hizası dengesi için)
  ...
  const beamStroke3D = Math.max((sections.beamDepth / 100) * isoScale, 2); // Biraz daha kalınlaştırdık
  ...
  return (
    <div className="w-full h-full grid grid-cols-1 md:grid-cols-3 gap-2 items-stretch overflow-hidden">
      ...
      {/* 1. KART: KAT PLANI */}
      <div className="flex flex-col items-center justify-center bg-white rounded-xl shadow-sm border border-slate-200 p-2 min-h-[240px]">
        ...
        <svg width={canvasSize} height={canvasSize} className="border border-slate-100 bg-slate-50/50 rounded">
          ...
          <text x={startX_Plan + widthPx / 2} y={startY_Plan + heightPx + 12} textAnchor="middle" className="text-[9px] fill-slate-500 font-mono">Lx={dimensions.lx}m</text>
          <text x={startX_Plan - 8} y={startY_Plan + heightPx / 2} textAnchor="middle" transform={`rotate(-90, ${startX_Plan - 8}, ${startY_Plan + heightPx / 2})`} className="text-[9px] fill-slate-500 font-mono">Ly={dimensions.ly}m</text>
        </svg>
      </div>
      ...
      <div className="flex flex-col items-center justify-center bg-white rounded-xl shadow-sm border border-slate-200 p-2 min-h-[240px]">
        ...
        <svg width={canvasSize} height={elevationHeight} className="border border-slate-100 bg-slate-50/50 rounded">
          ...
          <g transform={`translate(0, ${elevationHeight - 20}) scale(1, -1)`}>
            <line x1="10" y1="0" x2={canvasSize-10} y2="0" stroke="#94a3b8" strokeWidth="1.5" />
      ...
      {/* 3. KART: 3D PROFİL (GÜNCELLENMİŞ) */}
      <div className="flex flex-col items-center justify-center bg-white rounded-xl shadow-sm border border-slate-200 p-2 min-h-[240px]">
        <h3 className="text-slate-500 text-[10px] font-bold mb-2 tracking-wider uppercase flex items-center gap-2">
          <span className="w-2 h-2 rounded-full bg-orange-500"></span>
          3D Profil
        </h3>
        <svg width={canvasSize} height={canvasSize} className="border border-slate-100 bg-slate-50/50 rounded overflow-visible">
           <defs>
          ...
          {(() => {
           ...
           {Array.from({ length: storyCount }).map((_, i) => {
             ...
             // Kiriş Çizim Fonksiyonu (Soft Görünüm için)
             const drawBeam = (p1: {x:number, y:number}, p2: {x:number, y:number}) => (
               <g>
                 {/* 1. Gövde: Kalın, açık renk, yarı şeffaf (Hacim hissi) */}
                 <line 
                   ...
                   strokeLinecap="butt"
                 />
                 {/* 2. Hat: İnce, koyu renk (Netlik hissi) */}
                 <line 
                   x1={p1.x} y1={p1.y} x2={p2.x} y2={p2.y} 
                   stroke="#ea580c" 
                   strokeWidth={0.5} 
